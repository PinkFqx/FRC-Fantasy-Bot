...
function loadData() {
  try {
    ...
    return {
      ...
      currentPick: 0,
      phase: "none",
      draftOpen: false,           // false = closed, true = open for joining
      lastSeasonStandings: [],
      worldsTeams: [],
      seasonTeams: []
    };
...
client.on('interactionCreate', async (interaction) => {
  ...
  try {
    ...
    if (interaction.commandName === 'start_draft') {
      ...
      data.draftOpen = false;                    // close joining once draft starts
      saveData(data);
